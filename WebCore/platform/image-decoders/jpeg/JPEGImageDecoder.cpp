...
void JPEGImageDecoder::decode(bool onlySize)
{
	...
	//+EAWebKitChange
	// 5/2/2012 - Changed the allocation to be at 16 byte alignment. Microsoft documents that on x64, the set_jmp
	// buffer needs to be at least 16 byte aligned. Since by default an allocator may not be aligned to that boundary, we need to 
	// account for it.
	// We align JPEGImageReader cross-platform.
#if PLATFORM(EA)
	if (!m_reader)
		m_reader = adoptPtr(::new(16, 0, 0, 0, 0, 0, 0) JPEGImageReader(this));
#else
	if (!m_reader)
