...
//+EAWebKitChange
//5/1/2014
// This class represents a cross-platform interface that renders the render tree to tile buffers using the TiledBackingStoreClient and TiledBackingStoreBackend helper.
// TiledBackingStoreClient is WebCore::Frame and helps by connecting to the render tree. TiledBackingStoreBackend is tile buffer generator.
// When using TiledBackingStore, there is no need to explicitly call FrameView::Paint as it is done by this class. So this means that the dirty rects received by ChromeClient::invalidateContentsAndRootView
// have already been rendered. In regular flow, those dirty rects should be explicitly rendered by the port.
// The rects are usually in the content coordinates unless specified. 
//-EAWebKitChange
...
TiledBackingStore::TiledBackingStore(TiledBackingStoreClient* client, PassOwnPtr<TiledBackingStoreBackend> backend)
    ...
    //+EAWebKitChange
    //3/22/2012
	, m_useTimers(false)//Default to no timers
	, m_tilesNeedUpdate(true)
...
void TiledBackingStore::updateTilesIfNeeded(void)
{
	...
	// Figure out if we need tiles. This could be because it is first time or the page content changed (for example, loading a different page)
	// This also takes care of removing any unnecessary tiles (for example, when navigating to a new page with smaller content).
	// This calls createTiles but does not immediately create tiles. It marks the m_tilesNeedCreation to true and we create those tiles below.
	...
	// render the content into the tile buffer
...
//+EAWebKitChange
//5/1/2014
// RenderObject points to RenderView which points to FrameView. RenderObject tells RenderView that its rect should be repainted. RenderView uses FrameView::repaintContentRectangle to accomplish that. 

// In non-tiled mode, FrameView::repaintContentRectangle goes ahead and calls ScrollView::repaintContentRectangle which feeds the dirty rects to ChromeClient::invalidateContentsAndRootView in windows coordinates.
// The user of the FrameView then manually calls the Paint operation to have the content render. 

// In tiled backing storage mode, FrameView::repaintContentRectangle calls this invalidate method which marks the dirty rects inside individual tiles. Then, in updateTilesIfNeeded(),  those tiles are rendered first and 
// it feeds the dirty rects to ChromeClient::invalidateContentsAndRootView is window coordinates. 

// I think the reason for this somewhat opposite flows is that in the normal flow, you only want to render the content that is visible and only when you have created backing storage. In the tiled flow, you want to render the content regardless
// so that you can scroll faster. 

// Take the content dirty rect, figure out which tiles are intersecting and add pass it the dirty rect.
//-EAWebKitChange
...
//+EAWebKitChange
//5/1/2014
// First calculate layout etc. Then, figure out if you have any dirty rects from all the tiles that have not been painted yet.
// If yes, render the content in the tiles. The tiles pass back the dirty content coordinates (paintedArea)that were rendered. Pass them to the
// client which are then fed to ChromeClient::invalidateContentsAndRootView after converting them to window coordinates.
//-EAWebKitChange
...
void TiledBackingStore::paint(GraphicsContext* context, const IntRect& rect)
{
    ...
    // as the dirtyRect can span multiple tiles, go through each tile that intersect dirty rect and paint it
	for (int yCoordinate = topLeft.y(); yCoordinate <= bottomRight.y(); ++yCoordinate) {
        for (int xCoordinate = topLeft.x(); xCoordinate <= bottomRight.x(); ++xCoordinate) {
...
// Visible rect - visible viewport at the scroll location.
