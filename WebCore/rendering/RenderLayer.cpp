...
bool RenderLayer::paintsWithFilters() const
{
	...
	//+EAWebKitChange
	//3/10/2014 - Added a fix so that the filters can render without accelerated compositing support. Added some comments to explain current behavior in the composited path.
#if USE(ACCELERATED_COMPOSITING)
	if (!renderer().hasFilter())// No filters in layer, return false so no software mode kicks in.
		return false;

	if (!isComposited()) // The layer has filter but is NOT composited so return true to kick in software mode.
		return true;

	if (!m_backing || !m_backing->canCompositeFilters()) // The layer has filters, is composited, but the compositor can NOT render filter effects so return true to kick in software mode.
		return true;

	// At this point(if compositing is enabled), it safely determined that layer has filters, layer is actually composited, and the compositor can render filters. So move on and return false so that layer skips rendering filters in software mode.


#else
	// Kick in software mode if the layer has filters. 
