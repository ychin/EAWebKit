	...
	, mDisplayName(EA::Allocator::EASTLICoreAllocator("[EAUser] Display Name String", allocator))
{
	...
	GgpStatus status;
	EA_ASSERT_FORMATTED(GgpFutureGetResult(result, &profile, sizeof(GgpProfile), &status), ("Unable to get player profile from GGP - error %d", status.status_code));
	EA_UNUSED(result);
	EA_UNUSED(status);

	mUserIDString.append_convert(eastl::to_string(mUserID).c_str());
	mDisplayName.assign_convert(profile.gamer_tag, EA::StdC::Strlen(profile.gamer_tag));
}
