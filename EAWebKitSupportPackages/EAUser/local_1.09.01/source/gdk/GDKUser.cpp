	...
	, mXboxLiveContext(nullptr)
{
	...
	if (IsGuest())
	{
		mUserIDString.sprintf("Guest %d", mLocalID);
		mDisplayName.sprintf(u"Guest %d", mLocalID);
	}
	else
	{
		...
		EA_ASSERT_FORMATTED(SUCCEEDED(result), ("unable to retrieve gamertag for XUID %d", mXUID));

		mUserIDString.sprintf("%d", mXUID);
		mDisplayName.assign_convert(gamertag, static_cast<int>(length));

		result = XStoreCreateContext(mHandle, &mStoreContext);
		EA_ASSERT_FORMATTED(SUCCEEDED(result), ("XStoreCreateContext failed: %d", result));

		result = XblContextCreateHandle(user, &mXboxLiveContext);
		// We cannot assert this here, since if XBox Live has not yet been initialized, the call
		// will fail, and not all users of GDKUser will initialize XBox Live.  Assert in the
		// accessor below if the context is requested without having performed the initialization.
		//EA_ASSERT_FORMATTED(SUCCEEDED(result), ("XblContextCreateHandle failed: %d", result));
	}
}
...
UserOnlineState GDKUser::GetOnlineState() const
{
	...
	// BUG: MS forgot to update the GDK with a updated dll with GetNetworkConnectivityHint
