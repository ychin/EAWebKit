...
struct NetGameDistServT
{
    ...
    uint32_t uPrevStatClientCount;     //!< previous reported stat client count
...
void NetGameDistServUpdate(NetGameDistServT *pDistServ)
{
    ...
    if ((uSendThreshold == 0) || (pDistServ->uNbFramesNoData < uSendThreshold))
    {
        ...
        // check if client count changed between stat sampling interval
        if (pDistServ->uCurStatClientCount != pDistServ->uPrevStatClientCount)
        {
    ...
    // did flow control changed between stat sampling interval
    if (pDistServ->bPrevFlowEnabled != pDistServ->bFlowEnabled)
    {
        pDistServ->bFlowEnabledChanged = TRUE;
    }

    // did no input mode changed between stat sample inteval
    if (pDistServ->bPrevNoInputMode != pDistServ->bNoInputMode)
    {
...
int32_t NetGameDistServControl(NetGameDistServT *pDistServ, int32_t iControl, int32_t iValue, int32_t iValue2, void *pValue)
{
    ...
    if (iControl == 'rsta')
    {
        ...
        pDistServ->uPrevStatClientCount = pDistServ->uCurStatClientCount;
        pDistServ->bPrevFlowEnabled = pDistServ->bFlowEnabled;
        pDistServ->bPrevNoInputMode = pDistServ->bNoInputMode;
...
int32_t NetGameDistServStatus2(NetGameDistServT *pDistServ, int32_t iSelect, int32_t iValue, void *pBuf, int32_t iBufSize)
{
    ...
    if ((iSelect == 'drop') || (iSelect == 'icnt'))
    {
        ...
        {
            // value is not valid if number of client changed or flow control has changed between stat samples
            return(-1);
        }
    ...
    if (iSelect == 'ocnt')
    {
        if ((pDistServ->bClientCountChanged == TRUE) || (pDistServ->bNoInputModeChanged == TRUE) || (pDistServ->bNoInputMode == TRUE))
        {
            // value is not valid if number of client changed or flow control has changed between stat samples
            return(-1);
        }
