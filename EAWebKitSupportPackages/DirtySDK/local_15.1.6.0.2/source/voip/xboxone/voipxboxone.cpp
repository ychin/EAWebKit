...
struct VoipRefT
{
    ...
    int32_t                     iFirstPartyMuteCount;   //!< number of users in the voip session who have been muted by first party
};
...
/*F********************************************************************************/
/*!
    \Function    _VoipPollFirstPartyMuteCount

    \Description
        Poll for first party mute count changes and re-apply channel config if needed

    \Input *pVoip           - voip module reference

    \Version 6/03/2019 (tcho)
*/
/********************************************************************************F*/
static void _VoipPollFirstPartyMuteCount(VoipRefT *pVoip)
{
    int32_t iMuteCount = VoipHeadsetStatus(pVoip->Common.pHeadset, 'fpmc', 0, NULL, 0);

    if (iMuteCount != pVoip->iFirstPartyMuteCount)
    {
        pVoip->iFirstPartyMuteCount = iMuteCount;
...
static void _VoipThread(void *pParam)
{
    ...
    for (pVoip->iThreadActive = 1; pVoip->iThreadActive != 0; )
    {
        ...
        _VoipPollFirstPartyMuteCount(pVoip);
