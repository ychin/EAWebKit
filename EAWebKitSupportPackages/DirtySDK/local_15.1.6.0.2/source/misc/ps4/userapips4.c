...
#else
static void _UserAliOnWebApiUpdate(int32_t userCtxId, int32_t callbackId, const char *pNpServiceName, SceNpServiceLabel npServiceLabel, const SceNpPeerAddressA *pTo, const SceNpPeerAddressA *pFrom, const SceNpWebApi2PushEventDataType *pDataType, const char *pData, size_t dataLen, void *pUserArg)
#endif
{
...
#else
                if (!(DirtyWebApiRequest(pRef->pPlatformData->pWebApi, i, USERAPI_WEBAPI_GROUP, SCE_NP_WEBAPI2_HTTP_METHOD_GET, strPath, NULL, 0, NULL, _UserApiWebRequestCallback, pRef) >= 0))
#endif
                {
...
#else
        if (!(DirtyWebApiRequest(pRef->pPlatformData->pWebApiPresence, uUserIndex, USERAPI_WEBAPI_GROUP, SCE_NP_WEBAPI2_HTTP_METHOD_GET, strPath, NULL, 0, NULL, _UserApiWebPresenceRequestCallback, pRef) >= 0))
#endif
        {
            pRef->UserPresenceList[uUserIndex].iTotalErrors += 1;
            NetCritLeave(&pRef->crit);
...
#else
        if (DirtyWebApiRequest(pRef->pPlatformData->pWebApiRMP, uUserIndex, USERAPI_WEBAPI_RECENTLY_MET_GROUP, SCE_NP_WEBAPI2_HTTP_METHOD_POST, strPath, (uint8_t*)strPostData, strlen(strPostData), SCE_NP_WEBAPI2_CONTENT_TYPE_APPLICATION_JSON_UTF8, _UserApiWebRecentlyMetRequestCallback, pRef) < 0)
#endif
        {
...
#else
                           (data1Len > 0 ? SCE_NP_WEBAPI2_HTTP_METHOD_PUT : SCE_NP_WEBAPI2_HTTP_METHOD_DELETE), 
#endif
                           strPath, 
...
#else
                           (data1Len > 0 ? SCE_NP_WEBAPI2_CONTENT_TYPE_APPLICATION_JSON_UTF8 : NULL), 
#endif
