...
static int32_t _ProtoHttpFormatRequestHeader(ProtoHttpRefT *pHttpRef, const char *pUrl, const char *pHost, int32_t iPort, int32_t bSecure, const char *pRequest, int64_t iDataLen, const char *pAuthToken)
{
    ...
    iOffset += ds_snzprintf(pHttpRef->pInpBuf+iOffset, pHttpRef->iInpMax-iOffset, "%s %s HTTP/1.1\r\n", pRequest, pUrl);
...
static int32_t _ProtoHttpSendRequest(ProtoHttpRefT *pHttpRef)
{
    ...
    mbstowcs(pHttpRef->pWInpBuf, pHttpRef->strPath, sizeof(pHttpRef->strPath));
    ...
    // set the connection state (meaning initial connection and request)
...
static int32_t _ProtoHttpRead(ProtoHttpRefT *pHttpRef, char *pBuffer, int32_t iBufMin, int32_t iBufMax)
{
    ...
    if (iLen >= iBufMin)
    {
        ...
        // if we have read everything reset and kick off another query for more data
        if (pHttpRef->iInpOff == pHttpRef->iInpLen)
        {
            pHttpRef->iInpLen = pHttpRef->iInpOff = 0;
        }
...
static void _ProtoHttpUpdateInit(ProtoHttpRefT *pHttpRef)
{
    ...
    DWORD uFlags = WINHTTP_FLAG_ASYNC, uOption = 0;
    ...
    // $$TODO$$ move this to a control selector instead of basing the setting on DEBUG or not

    // when we are debugging we want to ability to use insecure websites, thus we don't set the secure flags
    #if !DIRTYCODE_DEBUG
    // require async and secure operating in XNetworking (secure includes async)
    uFlags = WINHTTP_FLAG_SECURE_DEFAULTS;
    #endif
    ...
    // startup winhttp
    if ((pHttpSession = WinHttpOpen(wstrUserAgent, WINHTTP_ACCESS_TYPE_AUTOMATIC_PROXY, NULL, NULL, uFlags)) == NULL)
    {
...
static void _ProtoHttpUpdateAuth(ProtoHttpRefT *pHttpRef)
{
    ...
    if ((iResult = DirtyAuthCheckToken(pHttpRef->iAuthTokenIdx, pTokenUrl, &iTokenLen, strAuthToken)) == DIRTYAUTH_SUCCESS)
    {
        ...
        if ((iResult = _ProtoHttpFormatRequestHeader(pHttpRef, pHttpRef->strPath, pHttpRef->strHost, pHttpRef->iPort, pHttpRef->bSecure, _ProtoHttp_strRequestType[pHttpRef->eRequestType], pHttpRef->iPostSize, strAuthToken)) >= 0)
        {
...
int32_t ProtoHttpRecv(ProtoHttpRefT *pHttpRef, char *pBuffer, int32_t iBufMin, int32_t iBufMax)
{
    ...
    if ((iBufMax = SocketRateThrottle(&pHttpRef->RecvRate, SOCK_STREAM, iBufMax, "recv")) == 0)
    {
        return(0);
    }
...
int32_t ProtoHttpRequestCb2(ProtoHttpRefT *pHttpRef, const char *pUrl, const char *pData, int64_t iDataSize, ProtoHttpRequestTypeE eRequestType, ProtoHttpWriteCbT *pWriteCb, ProtoHttpCustomHeaderCbT *pCustomHeaderCb, ProtoHttpReceiveHeaderCbT *pReceiveHeaderCb, void *pUserData)
{
    ...
    pUrlSlash = (*pUrl != '/') ? "/" : "";
    // set the path
    ds_snzprintf(pHttpRef->strPath, sizeof(pHttpRef->strPath), "%s%s", pUrlSlash, pUrl);
    ...
    if (bRecvToken == FALSE)
    {
        if ((iResult = _ProtoHttpFormatRequestHeader(pHttpRef, pHttpRef->strPath, strHost, iPort, bSecure, _ProtoHttp_strRequestType[eRequestType], iDataSize, NULL)) < 0)
        {
            return(iResult);
        }
