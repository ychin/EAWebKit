...
static int32_t _NetConnCommRestrictionRequest(NetConnRefT *pRef, int32_t iLocalUserIndex, NetConnAsyncRequestDataT *pRequestData)
{
    ...
    if (pRequestData->eRequestState != NETCONN_NP_REQUEST_STATE_INPROG)
    {
        ...
        iResult = DirtyWebApiRequest(pRef->pWebApi, iLocalUserIndex, NETCONNPS4_API_GROUP_COMMRESTRICTION, SCE_NP_WEBAPI2_HTTP_METHOD_GET, Request, NULL, 0, NULL, &_NetConnCommRestrictionRequestCallback, pRef);

        pRequestData->eRequestState = (iResult < 0) ? NETCONN_NP_REQUEST_STATE_FAILED : NETCONN_NP_REQUEST_STATE_INPROG;
    }
...
static int32_t _NetConnQueryPlayFirstTrial(void)
{
    ...
    {
        NetPrintf(("netconnps5: sceAppContentGetPftFlag() failed with error %s\n", DirtyErrGetName(iReturn)));
        iResult = -1;
    }
