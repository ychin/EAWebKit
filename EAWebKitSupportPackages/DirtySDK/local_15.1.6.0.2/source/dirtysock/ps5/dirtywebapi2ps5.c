...
/*!
    ...
    \Description
        DirtyWebApi2PS4 encapsulates use of the Sce's SessionInvitation API for 
...
typedef struct DirtyWebApi2PushEventT
{
    ...
    DirtyWebApi2EventCallbackEntryT CallbackEntry;      //!< callback entry
...
{
  struct DirtyWebApi2CallbackEntryT *pNext;
...
typedef struct DirtyWebApi2DereferedDeleteT
{
    int64_t iWebRequestId;
    uint32_t uTimeout;
    uint8_t bActive;
    uint8_t _pad[3];
} DirtyWebApi2DereferedDeleteT;
...
static int32_t _DirtyWebApi2CleanupRequestQueue(DirtyWebApiRefT *pRef, int32_t iUserIndex);
    ...
    size_t dataLen, const SceNpWebApi2PushEventExtdData *pExtdData, size_t extdDataNum, void *pUserArg)
{
    ...
    pEvent->CallbackEntry.pPushEventContextCallback(iUserCtxId, iCallbackId, pPushCtxId, cbType, NULL, SCE_NP_DEFAULT_SERVICE_LABEL, pTo, pFrom, pDataType, pData, dataLen, pEvent->pUserData);
}
...
{
    DirtyWebApi2PushEventT *pEvent = (DirtyWebApi2PushEventT *)pUserArg;
    pEvent->CallbackEntry.pPushEventContextCallback(iUserCtxId, iCallbackId, pPushCtxId, cbType, pNpServiceName, npServiceLabel, pTo, pFrom, pDataType, pData, dataLen, pEvent->pUserData);
}
...
{
    DirtyWebApi2PushEventT *pEvent = (DirtyWebApi2PushEventT*)pUserArg;
    pEvent->CallbackEntry.pPushEventCallback(iUserCtxId, iCallbackId, NULL, SCE_NP_DEFAULT_SERVICE_LABEL, pTo, pFrom, pDataType, pData, dataLen, pEvent->pUserData);
}
...
*/
/*************************************************************************************F*/
static void _DirtyWebApiCallbackNpWebApi2ServicePushEvent(int32_t userCtxId, int32_t callbackId, const char *pNpServiceName, SceNpServiceLabel npServiceLabel, 
...
{
    DirtyWebApi2PushEventT *pEvent = (DirtyWebApi2PushEventT*)pUserArg;
    pEvent->CallbackEntry.pPushEventCallback(userCtxId, callbackId, pNpServiceName, npServiceLabel, pTo, pFrom, pDataType, pData, dataLen, pEvent->pUserData);
}
...
static void _DirtyWebApi2DestroyPushEvents(DirtyWebApiRefT *pRef)
{
    ...
    for (int32_t iEvent = 0; iEvent < pRef->PushEvents.iEventCount; iEvent++)
    {
        for (int32_t iUser = 0; iUser < NETCONN_MAXLOCALUSERS; iUser++)
        {
            ...
            {
                if (pRef->PushEvents.aEvents[iEvent].CallbackEntry.eEventType == DIRTYWEBAPI2_PUSH_EVENT)
                {
                ...
                else if (pRef->PushEvents.aEvents[iEvent].CallbackEntry.eEventType == DIRTYWEBAPI2_PUSH_EVENT_CONTEXT)
                {
...
/*!
    ...
    \Input pNpServiceName   - NP web service generating the event
    \Input npServiceLabel   - more detail on the web service
    \Input pCallback        - DirtyWebApi2EventCallbackEntryT callback to call when even triggers
    \Input pUserData        - data passed to the callback when it triggers

    \Version 10/09/2013 (tcho)
*/
/*************************************************************************************F*/
static int32_t _DirtyWebApi2AddPushEventAsync(DirtyWebApiRefT *pRef, const char * eventType, const char * pNpServiceName, SceNpServiceLabel npServiceLabel, DirtyWebApi2EventCallbackEntryT *pCallback, void *pUserData)
{
    ...
    ds_memcpy(&pPushEvent->CallbackEntry, pCallback, sizeof(DirtyWebApi2EventCallbackEntryT));
    ...
    if (pRef->PushEvents.iEventCount < DIRTY_WEBAPI2_MAX_PUSH_EVENTS)
    {
        ...
        for (int32_t iUser = 0; iUser < NETCONN_MAXLOCALUSERS; iUser++)
        {
            ...
            if (NetConnStatus('soid', iUser, &onlineId, sizeof(onlineId)) >= 0)
            {
                ...
                {
                    if (pPushEvent->CallbackEntry.eEventType == DIRTYWEBAPI2_PUSH_EVENT)
                    {
                    ...
                    else if (pPushEvent->CallbackEntry.eEventType == DIRTYWEBAPI2_PUSH_EVENT_CONTEXT)
                    {
        ...
        ds_memcpy(&pRef->PushEvents.aEvents[pRef->PushEvents.iEventCount].CallbackEntry, &pPushEvent->CallbackEntry, sizeof(DirtyWebApi2EventCallbackEntryT));
...
/*F*************************************************************************************/
/*!
    \Function    _DirtyWevApiRemovePushEventListnenerCommon
...
*/
/*************************************************************************************F*/
static int32_t _DirtyWevApiRemovePushEventListnenerCommon(DirtyWebApiRefT *pRef, int32_t iRequestId, DirtyWebApi2EventCallbackEntryT *pCallback)
{
    ...
    for (int32_t i = 0; i < pRef->PushEvents.iEventCount; ++i)
    {
        ...
        {
            uint8_t bMatch = (pCallback->eEventType == DIRTYWEBAPI2_PUSH_EVENT) ? (pRef->PushEvents.aEvents[i].CallbackEntry.pPushEventCallback == pCallback->pPushEventCallback) : (pRef->PushEvents.aEvents[i].CallbackEntry.pPushEventContextCallback == pCallback->pPushEventContextCallback);
    ...
    if (iTarget < 0)
    {
        ...
        while (pPushEventRef != NULL)
        {
            ...
            {
                uint8_t bMatch = (pCallback->eEventType == DIRTYWEBAPI2_PUSH_EVENT) ? (pPushEventRef->CallbackEntry.pPushEventCallback == pCallback->pPushEventCallback) : (pPushEventRef->CallbackEntry.pPushEventContextCallback == pCallback->pPushEventContextCallback);
    ...
    else
    {
        ...
        for (int32_t iUser = 0; iUser < NETCONN_MAXLOCALUSERS; iUser++)
        {
            ...
            {
                if (pRef->PushEvents.aEvents[iTarget].CallbackEntry.eEventType == DIRTYWEBAPI2_PUSH_EVENT)
                {
                ...
                else if (pRef->PushEvents.aEvents[iTarget].CallbackEntry.eEventType == DIRTYWEBAPI2_PUSH_EVENT_CONTEXT)
                {
...
static void _DirtyWebApi2ShutdownWebApi(DirtyWebApiRefT *pRef)
{
    ...
    _DirtyWebApi2CleanupRequestQueue(pRef, -1);
...
*/
/*************************************************************************************F*/
static DirtyWebApi2WebRequestT *_DirtyWebApi2PopRequest(DirtyWebApiRefT *pRef, int32_t iUserIndex)
{
...
static int32_t _DirtyWebApi2CleanupRequestQueue(DirtyWebApiRefT *pRef, int32_t iUserIndex)
{
    ...
    //clear the queue
    while ((pWebRequest = _DirtyWebApi2PopRequest(pRef, iUserIndex)) != NULL)
    {
...
/*F********************************************************************************/
/*!
    \Function    _DirtyWebApi2AddCallbackToQueue
...
*/
/********************************************************************************F*/
static void _DirtyWebApi2AddCallbackToQueue(DirtyWebApiRefT *pRef, DirtyWebApiCallbackT *pCallback, int32_t iSceError, int32_t iUserIndex, int32_t iStatusCode, const char *pRespBody, int32_t iRespBodyLength, void *pUserData)
{
...
static void _DirtyWebApi2Thread(void *pArg)
{
    ...
    int32_t iRet = 0, iStatusCode, iBytesRead;
    char strThreadId[32];
    ...
    while (pRef->iThreadLife == 1)
    {
        ...
        pRef->pCurrentWebRequest = _DirtyWebApi2PopRequest(pRef, -1);
            ...
            pCurrentWebRequestLocal->iContentLength, &respInfoOption)) >= 0)
        {
            ...
            if ((iRet = _DirtyWebApi2ReadResponseData(pRef, pCurrentWebRequestLocal->iWebRequestId, pRef->aReadBuffer, pRef->uReadBufferSize - 1, &iBytesRead)) >= 0)
            {
                ...
                if (pRef->pCurrentWebRequest != NULL)
                {
                    _DirtyWebApi2AddCallbackToQueue(pRef, pRef->pCurrentWebRequest->pCallback, SCE_OK, pRef->pCurrentWebRequest->iUserIndex, iStatusCode, (iBytesRead ? pRef->aReadBuffer : NULL), iBytesRead, pRef->pCurrentWebRequest->pUserData);
                }
                NetCritLeave(&pRef->abortCrit);
            }
        }
        ...
        {            
            ...
            if (iRet < 0)
            {
                _DirtyWebApi2AddCallbackToQueue(pRef, pRef->pCurrentWebRequest->pCallback, iRet, pRef->pCurrentWebRequest->iUserIndex, 0, NULL, 0, pRef->pCurrentWebRequest->pUserData);
            }
...
DirtyWebApiRefT *DirtyWebApiCreate(const DirtyWebApi2CreateParamsT* pCreateParams)
{
    ...
    NetCritInit(&pRef->abortCrit, DIRTY_WEBAPI2_NET_HEAP_NAME);
...
int32_t DirtyWebApiRequestEx(DirtyWebApiRefT *pRef, int32_t iUserIndex, const char *pApiGroup, SceNpWebApi2HttpMethod eHttpMethod, const char *pPath, const uint8_t *pContent, int32_t iContentLength, const char *pContentType, const char *pReqHeaders, DirtyWebApiCallbackT *pCallback, void *pUserData)
{
        ...
        &iWebRequestId)) == SCE_OK)
    {
        ...
        if (pReqHeaders != NULL)
        {
            ...
            {
                if ((iResult = sceNpWebApi2AddHttpRequestHeader(iResult, strName, strValue)) != SCE_OK)
                {
                    NetPrintf(("dirtywebapi2ps5: could not add NpWebApi request header\n"));
                    sceNpWebApi2DeleteRequest(iResult);
                    return(iResult);
                }
            }
        }
...
int32_t DirtyWebApiAbortRequests(DirtyWebApiRefT *pRef, int32_t iUserIndex)
{
    if (pRef == NULL)
    {
        NetPrintf(("dirtywebapi2ps5: invalid module ref pointer\n"));
        return(-1);
    }
    ...
    return _DirtyWebApi2CleanupRequestQueue(pRef, iUserIndex);
}
...
{
    DirtyWebApi2EventCallbackEntryT Callback;
...
int32_t DirtyWebApiRemovePushEventListener(DirtyWebApiRefT *pRef, int32_t iRequestId, DirtyWebApiPushEventCallbackT *pCallback)
{
    ...
    Callback.pPushEventCallback = pCallback;
    Callback.eEventType = DIRTYWEBAPI2_PUSH_EVENT;
    return(_DirtyWevApiRemovePushEventListnenerCommon(pRef, iRequestId, &Callback));
}
