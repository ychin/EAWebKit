...
static void _NetConnShowAccountPicker(NetConnRefT *pRef)
{
    ...
    try
    {
        ...
        NetPrintf(("netconngdk: account picker opened\n"));
        ...
        ([pRef](IAsyncOperation<AccountPickerResult^>^ refAsyncOp, Windows::Foundation::AsyncStatus status)
        {
            switch(status)
            {
                ...
                {
                    NetPrintf(("netconngdk: [%p] _NetConnShowAccountPicker() An error occured in call to ShowAccountPickerAsync(), ErrorCode=0x%08x\n", pRef, refAsyncOp->ErrorCode));
                    break;
                }
                default:
                    break;
    ...
    {
        NetPrintf(("netconngdk: exception caught while trying open the account picker dialog (%S/0x%08x)\n", e->Message->Data(), e->HResult));
        pRef->eActPickerAsyncOpState = NetConnAccountPickerStateE::ST_ACTPICK_CLOSE;
    }
#endif
}
...
static void _NetConnGetEnvironment(NetConnRefT *pRef)
{
    ...
    {
        NetPrintf(("netconngdk: error %s getting sandbox id\n", DirtyErrGetName(hResult)));
        return;
    }
    ...
    {
        NetPrintf(("netconngdk: [%p] No Sandbox mapping found\n", pRef));
        return;
    }
