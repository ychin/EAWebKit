...
#define VOIPOPUS_RESAMPLE_BUFFER_SIZE ((20 * VOIPOPUS_DEFAULT_SAMPLING_RATE * sizeof(float)) / 1000)
...
static VoipCodecRefT *_VoipOpusCreate(int32_t iNumDecoders)
{
    ...
    pState->uSampleRateIn = VOIPOPUS_DEFAULT_SAMPLING_RATE;
...
static int32_t _VoipOpusEncodeBlock(VoipCodecRefT *pState, uint8_t *pOutput, const int16_t *pInput, int32_t iNumSamples)
{
    ...
    if (pOpus->uSampleRateIn != VOIPOPUS_DEFAULT_SAMPLING_RATE)
    {
...
static int32_t _VoipOpusControl(VoipCodecRefT *pState, int32_t iControl, int32_t iValue, int32_t iValue2, void *pValue)
{
    ...
    if (iControl == 'insr')
    {
        if (pOpus->uSampleRateIn != (uint32_t)iValue)
        {
            int32_t iError;
            
            //destroy old resampler
            if (pOpus->pSpeexResampler != NULL)
            {
                speex_resampler_destroy(pOpus->pSpeexResampler);
                pOpus->pSpeexResampler = NULL;
            ...
            if ((pOpus->pSpeexResampler = speex_resampler_init_frac(VOIPOPUS_DEFAULT_CHANNELS, pOpus->uSampleRateIn, VOIPOPUS_DEFAULT_SAMPLING_RATE, pOpus->uSampleRateIn, VOIPOPUS_DEFAULT_SAMPLING_RATE, VOIPOPUS_RESAMPLER_QUALITY, &iError)) == NULL)
            {
